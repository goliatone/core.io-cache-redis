...
class BatchCacheClient extends CacheClient {
    ...
    async tryGetBatch(keys, fallback, options = {}) {
        ...
        if (hasCacheKeys(getKeys)) {
            console.info('CacheClient:tryGetBatch fetching "%s"', getKeys);
        ...
        if (values.length === rawKeys.length) {
            console.info('CacheClient:tryGetBatch cached values are', values);
            return values;
        }

        try {
            console.log('CacheClient:tryGetBatch keys', batchKeys);
            /**
