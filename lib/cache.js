...
const ManagedSet = require('./data/set');

const defaults = require('./defaults').cache;
...
class CacheClient extends EventEmitter {
    ...
    managedSet(id) {
        let key = this.hashKey(id);
        if (this._msets.has(key)) return this._msets.get(key);
        let set = new ManagedSet(key, this.client);
        this._msets.add(key, set);
        return set;
    }
}
